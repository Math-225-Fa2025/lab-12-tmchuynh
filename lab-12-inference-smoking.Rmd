...
1. **Median-based approach:** Using the median age (approximately `r round(age_stats$median_age, 0)` years) creates two groups of equal size.
...
3. **Quartile approach:** Using the 75th percentile (`r round(age_stats$q75, 0)` years) separates the oldest quarter from the rest.
...
1. **Independence:** Observations are independent (different babies from different mothers)
2. **Sample Size:** Both groups have sufficient sample sizes
3. **Success-Failure:** Both success and failure counts exceed 5 in each group

**Conclusion:** The observed difference in proportions is `r round(obs_diff_prop, 4)`. The p-value is `r round(p_value_prop[[1]], 4)`. 

`r if(p_value_prop[[1]] < 0.05) { "Since p < 0.05, we reject the null hypothesis. There is statistically significant evidence that the proportion of low birth weight babies is higher for mature mothers compared to younger mothers." } else { "Since p ≥ 0.05, we fail to reject the null hypothesis. We do not have sufficient evidence to conclude that mature mothers have a higher proportion of low birth weight babies." }`

```{r exercise-12-confidence-interval}
...
**Confidence Interval Interpretation:** We are 95% confident that the true difference in proportions of low birth weight babies between mature and younger mothers is between `r round(ci_prop[[1]], 4)` and `r round(ci_prop[[2]], 4)`.

`r if(ci_prop[[1]] > 0) { "Since the confidence interval is entirely above zero, this provides strong evidence that mature mothers have a higher proportion of low birth weight babies. The practical significance of this difference can be assessed by the magnitude of the interval." } else if(ci_prop[[2]] < 0) { "Since the confidence interval is entirely below zero, this suggests younger mothers actually have a higher proportion of low birth weight babies." } else { "Since the confidence interval contains zero, this suggests that any difference in proportions may not be practically significant, even if we found statistical significance." }`
